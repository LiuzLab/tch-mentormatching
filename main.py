...
from bin.cache_utils import cache_key, save_to_cache, load_from_cache
...
def process_cv(file, num_candidates):
    try:
        ...
        search_results = search_candidate_mentors(
        ...
        for candidate, similarity_score in candidates:
            ...
            criterion_scores = extract_eval_scores_with_llm(client, match_res)
...
iface = gr.Interface(
    ...
    allow_flagging="never"  # Disable the flagging tab
)
