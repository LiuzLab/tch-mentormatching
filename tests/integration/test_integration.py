...
def faiss_index():
    ...
    documents = [
        Document(
            page_content="Mentor 1 summary",
            metadata={
                "Mentor_Profile": "mentor1.txt",
                "Professor_Type": "Professor",
                "Rank": 3,
            },
        ),
        Document(
            page_content="Mentor 2 summary",
            metadata={
                "Mentor_Profile": "mentor2.txt",
...
def setup_integration_test_environment(tmp_path):
    ...
    mock_index_summary_assistant_and_above = (
        db_dir / "index_summary_assistant_and_above"
    )
    ...
    with (
        ...
        patch(
            "src.config.paths.PATH_TO_MENTOR_DATA_RANKED",
            str(mock_mentor_data_ranked_csv),
        ),
        patch("src.config.paths.PROFESSOR_TYPES_PATH", str(mock_professor_types_txt)),
        patch(
            "src.config.paths.INDEX_SUMMARY_WITH_METADATA",
            str(mock_index_summary_with_metadata),
        ),
        patch(
            "src.config.paths.INDEX_SUMMARY_ASSISTANT_AND_ABOVE",
            str(mock_index_summary_assistant_and_above),
        ),
        patch(
            "src.config.paths.INDEX_SUMMARY_ABOVE_ASSISTANT",
            str(mock_index_summary_above_assistant),
        ),
    ):
        yield {
            "tmp_path": tmp_path,
...
@pytest.mark.asyncio
@patch(
    "src.retrieval.search_candidate_mentors.get_async_openai_client",
...
@patch(
    "src.retrieval.build_index.pd.read_csv"
)  # Patch pd.read_csv in src/retrieval/build_index.py
@patch("src.retrieval.build_index.OpenAIEmbeddings", lambda: FakeEmbeddings(size=1))
async def test_full_pipeline_integration(
    mock_read_csv_build_index,
    mock_summarize_cvs,
    mock_chat_openai,
    mock_load_dotenv,
    ...
    mock_read_csv_build_index.return_value = pd.DataFrame(
        {
            ...
            "Mentor_Summary": [
                "Mocked Summary for Mentor 1",
                "Mocked Summary for Mentor 2",
            ],
    ...
    await process_resumes_to_csv(
        str(env["resumes_dir"]), str(env["mock_mentor_data_csv"])
    )

    assert os.path.exists(str(env["mock_mentor_data_csv"]))

    # 2. Summarize CSVs
    await mock_summarize_cvs(
        str(env["mock_mentor_data_csv"]), str(env["mock_summarized_mentor_data_csv"])
    )
    ...
    mentee_summary_example = (
        "Mentee: Jane Smith, Skills: Python, Data Analysis, Research Interests: AI."
    )
    ...
    extracted_scores = await extract_eval_scores_with_llm(evaluation_text)
