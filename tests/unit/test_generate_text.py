...
def mock_tenacity_decorators():
    with patch('src.generate_text.retry', lambda *args, **kwargs: lambda fn: fn), \
...
async def test_completion_with_backoff_async(mock_openai_client):
    ...
    kwargs = {"model": "gpt-4", "messages": [{"role": "user", "content": "test"}]}
...
async def test_generate_text_async(mock_openai_client):
    ...
    mock_response_object.choices = [MagicMock(message=MagicMock(content="Generated Summary"))]
    ...
    expected_user_message = {"role": "user", "content": f"{instructions_input}\n{text_input}"}
    expected_messages = [expected_system_prompt, expected_user_message]

    summary = await generate_text_async(mock_openai_client, text_input, instructions_input)
    
    mock_openai_client.chat.completions.create.assert_called_once_with(
        model="gpt-4", messages=expected_messages, temperature=1.0
    )
