...
def mock_paths_fixture(tmp_path):
    ...
    paths_dict = {
        ...
        "PATH_TO_MENTOR_DATA_RANKED": str(mock_data_dir / "mentor_data_summaries_ranks.csv"),
        ...
        "INDEX_SUMMARY_ASSISTANT_AND_ABOVE": str(mock_db_dir / "index_summary_assistant_and_above"),
        "INDEX_SUMMARY_ABOVE_ASSISTANT": str(mock_db_dir / "index_summary_above_assistant"),
    }

    with patch('src.retrieval.build_index.paths', MagicMock(**paths_dict)) as mock_paths:
        yield mock_paths
...
def test_main_build_index_flow_with_existing_ranked_data(
    mock_open_file, mock_os_path_exists, mock_read_csv, mock_faiss, 
    mock_embeddings, mock_chat_openai, mock_load_dotenv, mock_paths_fixture
):
    # Arrange
    ...
    mock_os_path_exists.assert_called_once_with(mock_paths_fixture.PATH_TO_MENTOR_DATA_RANKED)
    mock_read_csv.assert_called_once_with(mock_paths_fixture.PATH_TO_MENTOR_DATA_RANKED, sep="\t")
